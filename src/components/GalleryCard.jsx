...
const HeroCard = ({ title, imageSrc, desc, btnTxt, btnCol }) => {
  return (
    <Link to="/" className="xl:w-1/4 m-xl:w-1/2 m-md:w-1/2 m-sm:w-full relative group hover:cursor-pointer overflow-hidden">
      <h3 className="font-means-web text-[26px] absolute py-4 px-5 font-semibold leading-9 group-hover:decoration-solid group-hover:underline z-10 text-white">{title}</h3>
      <img className="group-hover:scale-105 duration-200 h-[400px] w-full" src={imageSrc} />
